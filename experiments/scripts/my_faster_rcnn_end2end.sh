...
then
	time ./tools/train_net.py --gpu ${GPU_ID} \
	  --solver models/${PT_DIR}/${NET}/faster_rcnn_end2end/my_solver.prototxt \
	  --weights data/imagenet_models/${NET}.v2.caffemodel \
	  --imdb ${TRAIN_IMDB} \
	  --iters ${ITERS} \
	  --cfg experiments/cfgs/my_faster_rcnn_end2end.yml \
	  ${EXTRA_ARGS}
...
then
	time ./tools/test_net.py --gpu ${GPU_ID} \
	  --def models/${PT_DIR}/${NET}/faster_rcnn_end2end/my_test.prototxt \
	  --net ${NET_FINAL} \
	  --imdb ${TEST_IMDB} \
	  --cfg experiments/cfgs/my_faster_rcnn_end2end.yml \
	  ${EXTRA_ARGS}
