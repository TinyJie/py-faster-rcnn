...
then
	time ./tools/train_net.py --gpu ${GPU_ID} \
	  --solver models/${PT_DIR}/${NET}/faster_rcnn_end2end/solver_100.prototxt \
	  --weights data/imagenet_models/${NET}.v2.caffemodel \
	  --imdb ${TRAIN_IMDB} \
	  --iters ${ITERS} \
	  --cfg experiments/cfgs/faster_rcnn_end2end_224_224.yml \
	  ${EXTRA_ARGS}
...
then
	time ./tools/test_net.py --gpu ${GPU_ID} \
	  --def models/${PT_DIR}/${NET}/faster_rcnn_end2end/test_100.prototxt \
	  --net ${NET_FINAL} \
	  --imdb ${TEST_IMDB} \
	  --cfg experiments/cfgs/faster_rcnn_end2end_224_224.yml \
	  ${EXTRA_ARGS}
